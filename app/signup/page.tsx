...
export default async function SignupPage({
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-slate-50 dark:bg-slate-900">
      <div className="rounded-2xl bg-white dark:bg-slate-800 p-10 shadow-md w-full max-w-sm space-y-6 border border-slate-100 dark:border-slate-700">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 text-center">Opprett konto</h1>

        {error && (
          <p className="text-sm text-red-600 bg-red-50 dark:bg-red-900/20 rounded-lg px-4 py-2">{error}</p>
        )}
        {message && (
          <p className="text-sm text-green-600 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-2">{message}</p>
        )}

        {!message && (
          <form action={signup} className="space-y-4">
            <div className="space-y-1">
              <label htmlFor="display_name" className="text-sm font-medium text-slate-700 dark:text-slate-200">
                Ditt navn
              </label>
              <input
                id="display_name"
        ...
        <form action={loginWithGoogle}>
          <button
            type="submit"
            className="w-full py-2 px-4 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition font-medium text-sm flex items-center justify-center gap-2"
          >
        ...
        <p className="text-center text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">
          Har du konto?{" "}
          <a href="/login" className="font-medium text-slate-800 dark:text-slate-100 hover:underline">
            Logg inn
          </a>
        </p>
      </div>

...
function RecipeCard({
  ...
  return (
    <div className="border border-slate-100 dark:border-slate-700 rounded-xl overflow-hidden">
      ...
      >
        <span className="text-sm font-medium text-slate-800 dark:text-slate-100">{recipe.name}</span>
        <span className="text-slate-400 dark:text-slate-500 text-xs">{isOpen ? "▲" : "▼"}</span>
      </button>
            ...
            <div className="space-y-4">
              ...
              <div>
                <button
                  ...
                  className="flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition mb-2"
                >
                  Ingredienser
                  <span className="text-slate-300 dark:text-slate-600">{showIngredients ? "▲" : "▼"}</span>
                </button>
                {showIngredients && (
                      ...
                      <div key={cat.value}>
                        ...
                        <ul className="space-y-1">
                          {cat.items.map((ing) => {
                            ...
                            return (
                              <li key={ing.id} className="text-sm text-slate-700 dark:text-slate-200">
                                {ing.name}{" "}
              ...
              <div>
                ...
                >
                  Fremgangsmåte
                  <span className="text-slate-300 dark:text-slate-600">{showInstructions ? "▲" : "▼"}</span>
                </button>
                {showInstructions && (
                  <ol className="space-y-2 list-decimal list-inside">
                    {sortedInstructions.map((step) => (
                      <li key={step.id} className="text-sm text-slate-700 dark:text-slate-200">
                        {step.step_text}
                      </li>
                    ))}
                  </ol>
...
export default function RecipeList({
  ...
  if (recipes.length === 0) {
    return (
      <p className="text-slate-400 dark:text-slate-500 text-sm">Ingen oppskrifter lagt til ennå.</p>
    );
  }

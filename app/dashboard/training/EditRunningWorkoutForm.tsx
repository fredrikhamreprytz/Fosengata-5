...
import { updateRunningWorkout } from "../actions";
import type { RunningPhase, RunningWorkout, RunningStep } from "@/lib/types";
...
function stepToStepDraft(step: RunningStep): StepDraft {
  const durMin = step.duration_seconds !== null ? Math.floor(step.duration_seconds / 60) : 0;
  const durSec = step.duration_seconds !== null ? step.duration_seconds % 60 : 0;

  let paceMin = "";
  let paceSec = "";
  if (step.speed_kmh !== null && Number(step.speed_kmh) > 0) {
    const { min, sec } = speedToPace(Number(step.speed_kmh));
    paceMin = String(min);
    paceSec = String(sec).padStart(2, "0");
  }
  ...
  return {
    ...
    duration_minutes: durMin > 0 ? String(durMin) : "",
    duration_seconds: durSec > 0 ? String(durSec) : "",
    distance_km: step.distance_km !== null ? String(step.distance_km) : "",
    speed_kmh: step.speed_kmh !== null ? String(step.speed_kmh) : "",
    pace_min: paceMin,
    pace_sec: paceSec,
    incline_pct: step.incline_pct !== null ? String(step.incline_pct) : "",
  };
}
...
function initPhase(steps: RunningStep[], phase: RunningPhase): StepDraft[] {
  return [...steps]
...
export default function EditRunningWorkoutForm({
  workout,
  onDone,
}: {
  ...
  const [warmupSteps, setWarmupSteps] = useState<StepDraft[]>(() =>
    initPhase(workout.running_workout_steps, "warmup")
  );
  const [mainSteps, setMainSteps] = useState<StepDraft[]>(() =>
    initPhase(workout.running_workout_steps, "main")
  );
  const [cooldownSteps, setCooldownSteps] = useState<StepDraft[]>(() =>
    initPhase(workout.running_workout_steps, "cooldown")
  );
  const [showWarmup, setShowWarmup] = useState(() =>
    workout.running_workout_steps.some((s) => s.phase === "warmup")
  );
  const [showCooldown, setShowCooldown] = useState(() =>
    workout.running_workout_steps.some((s) => s.phase === "cooldown")
  );
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const phasePositions: Record<RunningPhase, number> = { warmup: 0, main: 0, cooldown: 0 };
    ...
    startTransition(async () => {
      const result = await updateRunningWorkout(workout.id, { name: name.trim(), steps });
      if (result.error) {
